...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTextColor = ta.getColorStateList(R.styleable.GridPasswordView_gpv_textColor);
        if (mTextColor == null)
            mTextColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.GridPasswordView_gpv_textSize, -1);
        if (textSize != -1) {
            this.mTextSize = px2sp(context, textSize);
        }
        mLineWidth = (int) ta.getDimension(R.styleable.GridPasswordView_gpv_lineWidth, dp2px(getContext(), 1));
        mLineColor = ta.getColor(R.styleable.GridPasswordView_gpv_lineColor, DEFAULT_LINECOLOR);
        mGridColor = ta.getColor(R.styleable.GridPasswordView_gpv_gridColor, DEFAULT_GRIDCOLOR);
        mLineDrawable = ta.getDrawable(R.styleable.GridPasswordView_gpv_lineColor);
        if (mLineDrawable == null)
            mLineDrawable = new ColorDrawable(mLineColor);
        mOuterLineDrawable = generateBackgroundDrawable();
        mPasswordLength = ta.getInt(R.styleable.GridPasswordView_gpv_passwordLength, DEFAULT_PASSWORDLENGTH);
        isVisible = ta.getBoolean(R.styleable.GridPasswordView_gpv_passwordVisible, true);
        mPasswordType = ta.getInt(R.styleable.GridPasswordView_gpv_passwordType, 0);
        ta.recycle();
    ...
    private void inflaterViews(Context context) {
        ...
        inflater.inflate(R.layout.view_grid_pwd, this);
        ...
        while (index < mPasswordLength) {
            View dividerView = inflater.inflate(R.layout.view_divider, this, false);
            ...
            TextView textView = (TextView) inflater.inflate(R.layout.view_textview, this, false);
    ...
    private GridEditText.OnDelKeyEventListener onDelKeyEventListener = new GridEditText.OnDelKeyEventListener() {
